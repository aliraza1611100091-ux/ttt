...
const KEYS_FILE = path.join(process.cwd(), 'clipdrop_keys.json');

export interface ClipdropKeyItem {
  id: string;
  key: string;
  usedCount: number;
  maxUses: number;
...
let clipdropKeys: ClipdropKeyItem[] = [];
    ...
    while (clipdropKeys.length > 0) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // If credit expired (402), unauthorized (401), or rate-limit quota (429), remove this key and advance to next key!
